...
# 课堂练习4.3（第1关）：fork 前后页式内存映射变化

DownloadSite=${1:-Gitee}
readonly DownloadSite

TaskName="4.3.ketang-1"
TmpDir="/tmp/${TaskName}"
...
case ${DownloadSite} in
  Gitee)
    ...
    download_file "${BaseUrl}/第一关.txt" "${TmpDir}/第一关.txt"
...
mv -f "${TmpDir}/第一关.txt" "/data/workspace/myshixun/第一关.txt"
mv -f "${TmpDir}/gdb.log" "/root/gdb.log"
mv -f "${TmpDir}/dbg.log" "/root/dbg.log"

echo "Checking generated files..."
ls -l "/data/workspace/myshixun/第一关.txt" "/root/gdb.log" "/root/dbg.log"

...
# 课堂练习4.4（第2关）：版本0内核第一次使用零页的缺页页故障分析

DownloadSite=${1:-Gitee}
readonly DownloadSite

TaskName="4.4.ketang-2"
TmpDir="/tmp/${TaskName}"
...
case ${DownloadSite} in
  Gitee)
    ...
    download_file "${BaseUrl}/第二关.txt" "${TmpDir}/第二关.txt"
...
mv -f "${TmpDir}/第二关.txt" "/data/workspace/myshixun/第二关.txt"
mv -f "${TmpDir}/gdb.log" "/root/gdb.log"
mv -f "${TmpDir}/dbg.log" "/root/dbg.log"

echo "Checking generated files..."
ls -l "/data/workspace/myshixun/第二关.txt" "/root/gdb.log" "/root/dbg.log"

...
# 课后作业4.1（第1关）：0号进程和1号进程的mynext变量的取值变化

DownloadSite=${1:-Gitee}
readonly DownloadSite

TaskName="4.1.kehou-1"
TmpDir="/tmp/${TaskName}"
...
case ${DownloadSite} in
  Gitee)
    ...
    download_file "${BaseUrl}/第三关.txt" "${TmpDir}/第三关.txt"
...
mv -f "${TmpDir}/第三关.txt" "/data/workspace/myshixun/第三关.txt"
mv -f "${TmpDir}/gdb.log" "/root/gdb.log"
mv -f "${TmpDir}/dbg.log" "/root/dbg.log"

echo "Checking generated files..."
ls -l "/data/workspace/myshixun/第三关.txt" "/root/gdb.log" "/root/dbg.log"
